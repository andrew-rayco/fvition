...
const RaceOptions = ({ race, intro }: RaceOptionsProps) => {
  ...
  const toggleHidden = (
    ...
    e.preventDefault();
    const raceDetails = document.getElementById(elementToToggle);
    const raceSection = document.getElementsByClassName(race.round)[0];
        ...
        <div className={`twelve columns round ${race.round}`}>
          <h4 onClick={(e) => toggleHidden(e, race.round)}>
            <button className='race-title'>
              {intro
                ? `${intro} the ${race.season} ${race.raceName}`
          ...
          <div id={race.round} className={`toggle hidden`}>
            ...
            <p>
              <button
                onClick={(e) => handleClick(e, 'qualiVisible')}
            ...
            {visibility.qualiVisible ? (
              <Quali season={race.season} round={race.round} />
            ) : null}

            <p>
              <button
                onClick={(e) => handleClick(e, 'gridVisible')}
                className='button-link'
              >
                Starting grid{' '}
                <img
            ...
            <p>
              <button
                onClick={(e) => handleClick(e, 'resultsVisible')}
                className='button-link'
              >
                Results{' '}
                <img
